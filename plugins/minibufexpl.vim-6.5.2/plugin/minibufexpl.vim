...
function! <SID>VimEnterHandler()
  ...
  let g:BufList = range(1, bufnr('$'))
  let s:MRUList = range(1, bufnr('$'))

  for l:i in g:BufList
    if <SID>IsBufferIgnored(l:i)
        call <SID>ListPop(g:BufList,l:i)
    endif
  endfor
...
function! <SID>BufAddHandler()
  ...
  call <SID>ListAdd(g:BufList,str2nr(expand("<abuf>")))
...
function! <SID>BufDeleteHandler()
  ...
  call <SID>ListPop(g:BufList,str2nr(expand("<abuf>")))
  call <SID>ListPop(s:MRUList,str2nr(expand("<abuf>")))

  call <SID>UpdateAllBufferDicts(expand("<abuf>"),1)

  " Handle ':bd' command correctly
  if (bufname('%') == '-MiniBufExplorer-' && <SID>NextNormalWindow() == -1 && len(g:BufList) > 0)
    if(tabpagenr('$') == 1)
      setlocal modifiable
      resize
      exec 'noautocmd sb'.g:BufList[0]
...
function! <SID>CycleBuffer(forward,...)
  ...
  if exists('a:1') && a:1 == 1
    ...
    call <SID>DEBUG('BufList is '.string(g:BufList),1)
    let curBufIndex = index(g:BufList, l:curBufNum)
    call <SID>DEBUG('curBufIndex is '.l:curBufIndex,1)
    let forBufIndex = l:curBufIndex + 1 >= len(g:BufList) ? 0 : l:curBufIndex + 1
    call <SID>DEBUG('forBufIndex is '.l:forBufIndex,1)
    let bacBufIndex = l:curBufIndex - 1 < 0 ? len(g:BufList) - 1 : l:curBufIndex - 1
    call <SID>DEBUG('bacBufIndex is '.l:bacBufIndex,1)
    ...
    if a:forward
      ...
      let l:moveCmd = 'b! '.g:BufList[l:forBufIndex]
    else
      if !g:miniBufExplCycleArround && l:curBufIndex < l:bacBufIndex
        echo "You have reached the first buffer!"
        return
      endif
      let l:moveCmd = 'b! '.g:BufList[l:bacBufIndex]
    endif
...
function! <SID>IsBufferIgnored(buf)
  ...
  if buflisted(a:buf) == 0 || index(g:BufList,a:buf) == -1
...
"  endif
  ...
  if getbufvar(a:buf, "&modifiable") == 0
    return 1
  endif
...
function! <SID>HasEligibleBuffers()
  ...
  let l:found = len(g:BufList)
  let l:needed = g:miniBufExplBuffersNeeded

  call <SID>DEBUG('Eligible buffers are '.string(g:BufList),6)
