...
function! Perl_Debugger ()
  ...
  let filename_win  = substitute(filename, '\\', '\\\\', "g")
  "
  if  s:MSWIN
  ...
  if v:windowid != 0
    ...
    if s:Perl_Debugger == "ptkdb"
      if  s:MSWIN
				exe '!perl -d:ptkdb "'.filename_win.l:arguments.'"'
      else
...
"	if has("gui_running")
"		"
"		noremap    <buffer>  <silent>  <S-F9>             :call Perl_SyntaxCheck()<CR>
"		inoremap   <buffer>  <silent>  <S-F9>        <C-C>:call Perl_SyntaxCheck()<CR>
"		"
"		noremap    <buffer>  <silent>  <S-F11>            :call Perl_Run()<CR>
"		inoremap   <buffer>  <silent>  <S-F11>       <C-C>:call Perl_Run()<CR>
"		"
"		noremap    <buffer>            <S-F10>            :PerlScriptArguments<Space>
"		inoremap   <buffer>            <S-F10>       <C-C>:PerlScriptArguments<Space>
"		"
" 		noremap    <buffer>  <silent>  <S-F1>             :call Perl_perldoc()<CR>
" 		inoremap   <buffer>  <silent>  <S-F1>        <C-C>:call Perl_perldoc()<CR>
"	endif
